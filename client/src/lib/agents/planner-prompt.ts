...
import type { AnalyzedStack } from "@/types/agent-schemas";

import type { ArchitectPreferences } from "@/types/agent-schemas";
...
export function extractJsonFromString(text: string): string {
  ...
  if (braceIndex > -1) {
    const braceEndIndex = text.lastIndexOf("}");
    if (braceEndIndex > braceIndex) {
      return text.substring(braceIndex, braceEndIndex + 1);
    }
  }
  return text;
}
...
/**
 * Generates the prompt for the "Architect" (Agent 1: Analyzer)
 */
export function generateArchitectPrompt(
  ...
  const constraints =
    preferences?.mode === "custom"
      ? `
# USER PREFERENCES
The user has provided these explicit preferences. Prioritize them.
- Framework: ${preferences.framework || "None"}
- UI: ${preferences.uiLibrary || "None"}
- Auth: ${preferences.authentication || "None"}
- DB: ${preferences.database || "None"}
- Deployment: ${preferences.deployment || "None"}
- Context: ${preferences.additionalContext || "None"}
`
      : `
# USER PREFERENCES
The user has not specified any tech. You are free to choose the best modern stack.
`;

  return `
You are an elite, 10x Solutions Architect. Your job is to read a startup blueprint 
and output the technical stack for the **full software solution** that an
autonomous executing agent will build.

## Your Reasoning Process:

1.  **Find the REAL Product:** Use the "machine-readable" tags.
    * Parse content inside * Parse content inside * Parse content inside * **IGNORE** all content inside blocks.

2.  **Analyze the "Solution":** Read the parsed sections to find all required features.

3.  **Infer Unstated Needs:** This is your most important job.
    * "SaaS" or "Agency Plan" -> requires **Payments** (e.g., Stripe)
    * "User accounts," "portfolio," "dashboard" -> requires **Database** (e.g., PostgreSQL)
    * "User accounts" -> requires **Authentication** (e.g., NextAuth.js)
    * "Connects to bank accounts" -> requires **External API** (e.g., Plaid)

4.  **Analyze Founder Skills:** Read the "FOUNDER_SKILLS" section.
    * If technical (e.g., "Nuxt developer"), you MUST use their skills (e.g., Nuxt).
    * If non-technical, pick the *best-in-class, modern, full-code stack* (e.g., Next.js 14, Supabase).
...
## Your Output
Respond ONLY with a valid JSON object. Do not add comments. This JSON defines
the full-code software product.
{
  "framework": { "name": "Next.js 14", "rationale": "Best for modern SaaS dashboards." },
  "ui": { "name": "Shadcn UI + Tailwind", "rationale": "Modern, beautiful components." },
  "database": { "name": "Supabase (PostgreSQL)", "rationale": "Inferred need for user accounts." },
  "authentication": { "name": "Supabase Auth", "rationale": "Inferred need for user accounts." },
  "payments": { "name": "Stripe", "rationale": "Inferred from 'SaaS' model." },
  "external_apis": { "name": "Plaid", "rationale": "Core requirement from 'Solution'." },
  "hosting": { "name": "Vercel", "rationale": "Best-in-class for Next.js." }
}
`;
}
...
- Modern full-stack architecture (React, Vue, Angular, Svelte, Node.js)
...
export function generatePlanningPrompt(
  ...
  // 🔽 BUILD THE NEW "APPROVED STACK" INJECTION 🔽
  let approvedStackSection = `
## ✅ APPROVED TECH STACK

You are the Planner. The Architect has already analyzed the blueprint and
approved the following tech stack. You MUST build the entire atomic plan
using ONLY these technologies. Do not deviate.

- **Framework:** ${analyzedStack.framework.name}
  - *Rationale:* ${analyzedStack.framework.rationale}
- **UI:** ${analyzedStack.ui.name}
  - *Rationale:* ${analyzedStack.ui.rationale}
- **Database:** ${analyzedStack.database.name}
  - *Rationale:* ${analyzedStack.database.rationale}
- **Authentication:** ${analyzedStack.authentication.name}
  - *Rationale:* ${analyzedStack.authentication.rationale}
- **Hosting:** ${analyzedStack.hosting.name}
  - *Rationale:* ${analyzedStack.hosting.rationale}
`;

  if (analyzedStack.payments) {
    approvedStackSection += `
- **Payments:** ${analyzedStack.payments.name}
  - *Rationale:* ${analyzedStack.payments.rationale}
`;
  }
let researchSection = "";
if (researchedStack) {
  researchSection = `
## 📚 TOP-TIER RESEARCH
A research agent has compiled the following up-to-date documentation.
You MUST prioritize these guides in your plan.

${JSON.stringify(researchedStack, null, 2)}
---
`;
}

// ⬇️ MODIFY THE RETURN STRING ⬇️
return `
...
${researchSection} // 🆕 INJECT THE RESEARCH
...
1. **Analyze the Blueprint AND the Approved Stack**
   - Identify core features and user flows
   - Determine required integrations (auth, payments, APIs)

2. **Design the Architecture**
   - **CRITICAL: You MUST use the APPROVED TECH STACK above.**
   - All file paths, install commands, and patterns must match the approved stack.
   - (e.g., If stack is Nuxt, use \`npx nuxi init\`, \`useAsyncData\`, \`server/api\`)
   - (e.g., If stack is Next.js, use \`npx create-next-app\`, Server Actions, \`app/api\`)

3. **Create Atomic Build Plan**
   - Break the project into 15-40 atomic tasks
   - Group into logical phases (Foundation, Auth, Core Features, etc.)
   - Each task should:
