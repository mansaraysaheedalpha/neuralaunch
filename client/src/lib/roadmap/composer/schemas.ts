...
import { DispatchPlanSchema } from "./dispatch-plan-schema";
...
export const ComposerOutputSchema = z.object({
  ...
  /** Optional only so sessions generated before dispatch plans remain readable. */
  dispatchPlan: DispatchPlanSchema.optional(),
});
export type ComposerOutput = z.infer<typeof ComposerOutputSchema>;

/** Required generation shape for all newly-created Composer sessions. */
export const GeneratedComposerOutputSchema = z.object({
  messages: z.array(ComposerMessageSchema),
  dispatchPlan: DispatchPlanSchema,
});
 ...
 */
export function safeParseComposerSession(
  value: unknown,
