  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-2xl mb-10">
          <div className="rounded-xl border border-gold/20 bg-gold/5 px-6 py-4 text-center">
            ...
            <p className="mt-2 text-sm text-slate-300 leading-relaxed">
              First 50 users: Execute at <span className="text-gold font-semibold">$19/month</span> forever.
              Compound at <span className="text-gold font-semibold">$29/month</span> forever.
            </p>
      ...
      <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {TIERS.map((tier) => {
          const borderClass =
            tier.accent === 'primary' ? 'border-primary shadow-lg shadow-primary/10' :
            tier.accent === 'gold'    ? 'border-gold/40 shadow-lg shadow-gold/10' :
            'border-slate-800';
          const bgClass =
            tier.accent === 'primary' ? 'bg-navy-800' :
            tier.accent === 'gold'    ? 'bg-navy-800' :
            'bg-navy-900';
          const badgeClass =
            tier.accent === 'gold'
              ? 'bg-gold text-white'
          ...
          const pricing = isPaid ? pricingByTier[tier.name as 'Execute' | 'Compound'] : null;
          const priceId = pricing ? (cycle === 'annual' ? pricing.annual : pricing.monthly) : null;
          // Founding rate only applies to monthly — mirrors spec §1.2.
          const showFoundingNote = pricing?.isFoundingRate && cycle === 'monthly';
          ...
          return (
            ...
            >
              {tier.badge && (
                <span className={`absolute -top-3 left-7 inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wider ${badgeClass}`}>
                  {tier.badge}
                </span>
              )}
                ...
                {price === 0 ? (
                  ...
                  <>
                    <p className="text-3xl font-bold text-white">
                      ${Math.round(price / 12)}<span className="text-lg font-medium text-slate-400">/mo</span>
                    </p>
                    <p className="mt-1 text-xs text-slate-400">
                      {formatPrice(price, 'annual')} billed annually{' '}
                      <span className="text-success font-medium">— save ${savings}</span>
                    </p>
                  </>
                ) : (
                  <>
                    <p className="text-3xl font-bold text-white">
                      ${tier.name === 'Execute' && showFoundingNote ? 19
                        : tier.name === 'Compound' && showFoundingNote ? 29
                        : price}<span className="text-lg font-medium text-slate-400">/mo</span>
                    </p>
                    {showFoundingNote && (
                      <p className="mt-1 text-xs text-gold">
                        Founding member rate — ${price}/mo after launch
                      </p>
                    )}
                  </>
                )}
