...
export function ShortlistView({ document: doc }: ShortlistViewProps) {
  ...
  return (
    <div className="space-y-6">
      <section>
        <header className="mb-3">
          <h2 className="text-sm font-semibold text-fg">
            Shortlist <span className="text-muted">({shortlist.length} of up to {doc.shortlistCap})</span>
          </h2>
        ...
        <ol className="space-y-3">
          {shortlist.map((p, i) => (
            <li key={p.id} className="rounded-lg border border-rule bg-bg-2/30 px-3 py-3">
              <div className="flex items-start gap-3">
                <span className="mt-0.5 inline-flex size-6 shrink-0 items-center justify-center rounded-full bg-accent/10 text-xs font-mono text-accent">
                  {i + 1}
                </span>
      ...
      <section>
        <h2 className="text-sm font-semibold text-fg mb-2">Why these and not the others</h2>
      ...
      {rejected.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-fg mb-2">
            Considered but not shortlisted <span className="text-muted">({rejected.length})</span>
          </h2>
          <ul className="space-y-2">
            {rejected.map(p => (
              <li key={p.id} className="rounded-md border border-rule bg-bg-2/20 px-3 py-2 text-sm">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-xs text-muted mb-1">
        <span className="rounded bg-bg-3/60 px-1.5 py-0.5 font-medium uppercase tracking-wider">
      ...
      <p className="text-sm text-fg leading-snug">{painPoint.description}</p>
      {!compact && !isFounder && painPoint.evidenceExcerpt && (
        <blockquote className="mt-1.5 border-l-2 border-rule pl-2 text-xs text-muted">
      ...
      {!compact && isFounder && painPoint.founderNotes && (
        <p className="mt-1 text-xs text-muted italic">{painPoint.founderNotes}</p>
      )}
    </div>
  );
}
