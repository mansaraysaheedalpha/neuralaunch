    ...
    <main className="min-h-screen bg-bg text-fg">
      ...
      <section className="mx-auto max-w-2xl px-6 pb-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-rule bg-bg-2 p-6">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted">The friction</h2>
          <p className="text-sm text-fg leading-relaxed">{content.problemStatement}</p>
        </div>
        <div className="rounded-xl border border-accent/20 bg-accent/5 p-6">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-accent/70">The bridge</h2>
      ...
      <section className="mx-auto max-w-2xl px-6 pb-12 flex flex-col gap-4">
        <h2 className="text-sm font-semibold text-muted">Coming features — tell us what you need most</h2>
      ...
      <section className="mx-auto max-w-2xl px-6 pb-16">
        {!signedUp ? (
          ...
          <div className="rounded-xl border border-accent/30 bg-accent/5 px-6 py-5 text-center">
            <p className="text-sm font-medium text-accent">You're on the list — we'll reach out when we launch.</p>
          </div>
        )}
      </section>
    </main>
