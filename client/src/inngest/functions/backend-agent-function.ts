  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        // Task status already updated by BaseAgent
...
---
*Generated by NeuraLaunch Backend Agent with full framework support: tools, memory, dynamic retry, and error recovery.*
            `,
            githubToken: env.GITHUB_TOKEN!,
          });
      ...
      await step.run("check-wave-completion", async () => {
        ...
        log.info(
          `[Backend Agent] Wave ${waveNumber} progress: ${completedCount}/${totalCount}`
        );
        ...
        if (completedCount === totalCount) {
          log.info(
            `[Backend Agent] All Wave ${waveNumber} tasks complete! Triggering quality checks`
          );
