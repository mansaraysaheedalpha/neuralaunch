...
export function ReactivateDialog({
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-rule bg-bg-2 p-6 shadow-xl">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <h2 id="reactivate-dialog-title" className="text-sm font-semibold text-fg">
          ...
          <button
            ...
            onClick={onCancel}
            disabled={submitting}
            className="rounded-md p-1 text-muted hover:bg-bg-3 hover:text-fg disabled:opacity-50"
            aria-label="Cancel"
          >
            <X className="size-4" />
          </button>
        ...
        <p className="mt-3 text-xs text-muted leading-relaxed">
          You can have <span className="font-semibold text-fg">{cap}</span> active{' '}
          {cap === 1 ? 'venture' : 'ventures'} on your {tierLabel} plan. Activating{' '}
          <span className="font-medium text-fg">&ldquo;{activateName}&rdquo;</span>{' '}
          ...
          {activeOptions.map(opt => (
            <li key={opt.id}>
              <label className={`flex items-center gap-2.5 rounded-lg border px-3 py-2.5 cursor-pointer transition-colors ${
                ...
                />
                <span className="text-sm text-fg truncate">{opt.name}</span>
              </label>
            </li>
          ))}
        </ul>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            className="rounded-md bg-accent px-3 py-1.5 text-xs font-semibold text-bg hover:bg-accent/90 disabled:opacity-50 disabled:cursor-not-allowed"
          >
