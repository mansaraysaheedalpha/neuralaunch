...
export function TaskMetadata({ task }: { task: StoredRoadmapTask }) {
  return (
    <div className="flex flex-col gap-5">
      <p className="text-base text-foreground/90 leading-relaxed break-words">{task.description}</p>
      ...
      {/* WHY THIS MATTERS — gold-tinted callout. Bigger eyebrow +
          larger italic body to match the design tool's prominent
          treatment. The rationale is the most important context on
          a task because it answers the founder's recurring "why am
          I doing this" question at week 4 of execution. */}
      {task.rationale && (
      ...
      {/* Resource chips — each pill carries a small Link icon so the
          founder reads them as "things to open." */}
      {task.resources && task.resources.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {task.resources.map((r, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-card border border-border rounded-md px-2.5 py-1.5"
            >
