...
export default function ChatPage() {
  ...
  // Simplified useEffect for loading chat history
  useEffect(() => {
    const loadChatHistory = async () => {
      // A valid conversationId is a CUID, not 'new' or empty.
      if (!conversationId || conversationId === "new") {
        resetStore(); // Clear everything for a fresh start.
        return;
      }

      // Prevent re-fetching if we already have the data for this conversation.
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const isNewConversation =
        res.headers.get("X-Is-New-Conversation") === "true";

      if (isNewConversation && newConversationId) {
        router.push(`/chat/${newConversationId}`, { scroll: false });
          ...
          <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            <div className="space-y-6">
              {error && !isLoading && messages.length === 0 && (
                <div className="max-w-4xl mx-auto p-4 bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-2xl animate-shake">
