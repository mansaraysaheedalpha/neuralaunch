...
const SHELL = {
  ...
  heading: <>Ask in plain language.<br /><em>Get back structured truth.</em></>,
  lede: <>A multi-step research pass across the open web. Every finding carries its <em>sources</em> and a confidence label — verified, likely, or unverified. No confident-sounding guesses.</>,
};
...
function ResearchLoaded({
  ...
  const taskScoped = typeof window !== 'undefined'
    ...
    <ToolShell {...SHELL} flush>
      ...
      <div className="grid min-h-[calc(100vh-56px)] grid-cols-1 lg:grid-cols-[1fr_1.1fr]">
        ...
        <div className="border-b border-rule lg:border-b-0 lg:border-r lg:border-rule px-6 pb-20 pt-10 sm:px-12">
          ...
          {flow.error && (
            <p className="mb-6 border-l-2 border-amber bg-bg-2 px-4 py-2.5 font-mono text-[10px] uppercase tracking-[0.14em] text-amber">
              {flow.error}
            </p>
          )}
          ...
          {isPlanning && (
            <div className="flex flex-col items-start gap-3 py-2">
              ...
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
                Opus is decomposing the question and naming the sources to search.
              </p>
            </div>
          )}
          ...
          {isPlanReview && (
            <PlanReview
              ...
              onApprove={(p) => { void flow.handlePlanApprove(p); }}
          ...
          {isExecuting && (
            <>
              {jobStage === 'failed' ? (
                <ExecutionFailed
                  message={flow.executeJob?.errorMessage ?? 'Research run failed.'}
        ...
        <div className="px-6 pb-20 pt-10 sm:px-12">
          ...
          {isReport && flow.report && (
            <FindingsLedger
              ...
              onAttachToTask={taskScoped ? () => { /* persisted on accept */ } : undefined}
            />
          )}
...
type StageLeftStatus =
  | 'Ready' | 'Planning…' | 'Plan ready' | 'Researching…' | 'Complete';
...
function EmptyOutput() {
  return (
    <div className="flex flex-col items-center gap-4 border border-dashed border-rule-strong px-8 py-14 text-center">
      <span aria-hidden="true" className="font-serif text-[64px] leading-none italic text-muted-2">⁇</span>
...
function WorkingOutput({ query }: { query: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 border border-rule-strong bg-bg-2 px-6 py-7">
      <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent">
        <Loader2 aria-hidden="true" className="inline-block size-3 mr-2 animate-spin" />
      ...
      <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
        Elapsed · {Math.floor(elapsed / 60)}m {String(elapsed % 60).padStart(2, '0')}s · findings appear when the run completes.
      </p>
    </div>
  );
}

function ExecutionFailed({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col gap-4 border-l-2 border-amber bg-bg-2 px-5 py-4">
      <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-amber">
        Research · didn&rsquo;t finish
      </p>
      ...
      <button
        type="button"
        onClick={onRetry}
        className="self-start inline-flex items-center gap-2 bg-accent px-4 py-2.5 font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-bg transition-opacity hover:opacity-90"
      >
        Retry research
