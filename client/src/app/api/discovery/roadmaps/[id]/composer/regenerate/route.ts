...
export async function POST(
  ...
  try {
    ...
    await rateLimitByUser(
      userId,
      "composer-standalone-regenerate",
    ...
    const log = logger.child({
      route: "POST composer-standalone-regenerate",
      roadmapId,
      userId,
    });
    ...
    const rawSessions: Array<Record<string, unknown>> = Array.isArray(
      roadmap.toolSessions,
    )
      ? (roadmap.toolSessions as Array<Record<string, unknown>>)
      : [];

    const rawSession = rawSessions.find(
      (s) => s["id"] === parsed.data.sessionId,
    );
    ...
    if (!session?.output)
      throw new HttpError(
        409,
        "No generated output found. Run generate first.",
      );

    const msgIndex = session.output.messages.findIndex(
    ...
    if (variationCount >= MAX_REGENERATIONS_PER_MESSAGE) {
      throw new HttpError(
        409,
        `Regeneration limit reached (${MAX_REGENERATIONS_PER_MESSAGE} variations maximum).`,
      );
    }

    const channelParsed = z.enum(COMPOSER_CHANNELS).safeParse(session.channel);
    if (!channelParsed.success)
      throw new HttpError(409, "Session channel is invalid.");
    ...
    const updatedSession = {
      ...
      output: {
        ...
        dispatchPlan: session.output.dispatchPlan,
      },
      updatedAt: new Date().toISOString(),
    };

    const otherSessions = rawSessions.filter(
      (s) => s["id"] !== parsed.data.sessionId,
    );
    ...
    log.info("[StandaloneComposer] Variation persisted", {
